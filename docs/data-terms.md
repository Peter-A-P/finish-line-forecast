...
**1.4 degrees cold** (sd 2.4) and **7.7 km/h calm** (sd 5.3) against the ECCC observation. The
...
Fetched by `finishline forecast-error`: one request per year of forecasts, one a second, with
the identifying user agent, kept under `data/cache/openmeteo/`. A live forecast is fetched at
